 ...
 * (application/x-www-form-urlencoded) whenever a user removes the app via
 * their Facebook/Instagram settings. We must:
 ...
 *
 * This is separate from the existing /data-deletion informational page
 * (which remains unchanged) — Meta calls this endpoint programmatically,
 * the other one is the human-readable instructions page linked from Settings.
 *
 * Secret resolution mirrors /api/webhooks/meta/instagram (env var first,
 * falling back to the platform_settings row) since both are verifying
 * requests against the same Meta App.
 */
